...
void getServerCommand() {
  ...
  delay(1000);  // Просто ждём вместо waitForResponse
  
  // 2. Очищаем буфер приёма
  ...
  if (!gotPrompt) {
    Serial.println(F("No > prompt, closing"));
    WIFI_SERIAL.println("AT+CIPCLOSE");
    return;
  }
  
  // 6. Отправляем GET запрос
